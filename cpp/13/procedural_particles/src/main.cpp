...
// Particle simulation
constexpr GLuint gMaxNumberOfParticles  = 15;
GLuint gVertexBufferParticleData        = 0;
...
void VertexSpecification(){
	...
	gTexture.LoadTexture("./ParticleAtlas.ppm");
    ...
    // Now let's setup a buffer for the particles size and positions
    glGenBuffers(1, &gVertexBufferParticleData);
    glBindBuffer(GL_ARRAY_BUFFER, gVertexBufferParticleData);
    // The trick here is that we are going to allocate on the GPU a buffer big enough
    // for all of our particles.
    // Then we want to store '4' pieces of floating point information: x,y,z, and size.
    // The NULL is because we are goint to 'stream in' the data and update
    // our buffer dynamically
    glBufferData(GL_ARRAY_BUFFER, gMaxNumberOfParticles * 4 * sizeof(GLfloat), NULL, GL_STREAM_DRAW);

    // Setup an attribute for this new data set
    // IMPORTANT NOTE: Keep in mind this is a new 'glGenBuffer', so our offsets
    // are for the specific glGenBuffer here -- not the previous one
    ...
    glVertexAttribPointer(2,
                          ...
                          GL_FALSE,
                          sizeof(GL_FLOAT)*4,
                          (GLvoid*)(sizeof(GL_FLOAT)*0)
            );


    // When we setup the 'attribute divisor' we are specifying that per 'instance' (gl_instanceID) in
    // our vertex shader, which buffer data to use
    glVertexAttribDivisor(0,0); // Use the same vertices (advances per vertex)
    glVertexAttribDivisor(1,0); // Use the same texture coordinates (advances per vertex)
    glVertexAttribDivisor(2,1); // 1 set of positions and size per quad (1=advances per instance, 2=advances per 2 instances)
...
void PreDraw(){
    ...
    // Setup the current frame to sample from in the currently loaded diffuse texture for
    // the textured quad
    static int particleFrame = 0;

    GLint u_particleFrameLocation = glGetUniformLocation(gGraphicsPipelineShaderProgram,"u_particleFrame");
    glUniform1i(u_particleFrameLocation,particleFrame);

    GLint u_dimensionsXLocation = glGetUniformLocation(gGraphicsPipelineShaderProgram,"u_dimensionsX");
    glUniform1i(u_dimensionsXLocation,8);

    GLint u_dimensionsYLocation = glGetUniformLocation(gGraphicsPipelineShaderProgram,"u_dimensionsY");
    glUniform1i(u_dimensionsYLocation,8);
    ...
    // Stack allocated positions
    static float particleData[gMaxNumberOfParticles*4];
    float step = 0.0f;
    for(int i=0; i < 100 * 4 ; i+=4){
        // Setup the lifetime
        if(particleData[i+3]==0.0f){
            particleData[i+3] = 1.0f;
        }
        particleData[i+3] -= 0.01f;
        // Set the positions
        particleData[i+0] = cos(float(step));
        particleData[i+1] = sin(float(step));
        particleData[i+2] = 0.0f;
    ...
    glBindBuffer(GL_ARRAY_BUFFER, gVertexBufferParticleData);
    // Trick with buffer update to avoid synchronization issues
    // https://www.khronos.org/opengl/wiki/Buffer_Object_Streaming#Buffer_re-specification 
    // Search 'orphaning'
    glBufferData(GL_ARRAY_BUFFER, gMaxNumberOfParticles * 4 * sizeof(GLfloat), NULL, GL_STREAM_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0, gMaxNumberOfParticles * 4 * sizeof(GLfloat), particleData);
...
*/
void Draw(){
    // Enable our attributes and the buffers that we are bound to
	glBindVertexArray(gVertexArrayObject);
...
//  glDrawElements(GL_TRIANGLES,6, GL_UNSIGNED_INT,0);
    glDrawElementsInstanced(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0,gMaxNumberOfParticles);
