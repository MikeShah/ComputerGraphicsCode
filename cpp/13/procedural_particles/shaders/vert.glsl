...
{	
    ...
    instancePosition.x += particleData[0];
    instancePosition.y += particleData[1];
    instancePosition.z += particleData[2];

    vec4 newPosition = u_Projection * u_ViewMatrix * u_ModelMatrix * vec4(instancePosition,1.0f);
                                                                    // Don't forget 'w'
