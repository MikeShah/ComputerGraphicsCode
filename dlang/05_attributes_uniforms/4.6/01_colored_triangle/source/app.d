...
void VertexSpecification(){
  ...
  // Stride here is '6' floats
	glVertexArrayVertexBuffer(gMesh.mVAO,0,gMesh.mVBO,0,GLfloat.sizeof*6); 
	// Vertex attributes
	glEnableVertexArrayAttrib(gMesh.mVAO,0); // Attribute #0 -- position
	glEnableVertexArrayAttrib(gMesh.mVAO,1); // Attribute #1 -- color

	// Associate VAO attribute
	glVertexArrayAttribBinding(gMesh.mVAO,0,0); // #0 -- position, then '0th' VBO
	glVertexArrayAttribBinding(gMesh.mVAO,1,0); // #1 -- color, then '0th' VBO

	// Set up each attribute
	// Attribute #0
	glVertexArrayAttribFormat(gMesh.mVAO,
                            0,               // Attribute number (#0 -- position)
                            3,               // Size
                            GL_FLOAT,        // Type
                            GL_FALSE,        // Normalized
                            cast(GLvoid*)0); // Offset
	// Attribute #1
  // Importantly need to set the 'offset' for where to read the data
  // Within the 'glVertexArrayVertexBuffer' the 'stride' is already set to 6.
	glVertexArrayAttribFormat(gMesh.mVAO,1, 3, GL_FLOAT, GL_FALSE, cast(GLvoid*)(GLfloat.sizeof*3));
}
