...
* SetupMVPMatrix 
* 
* Setup the Model, View, and Projection Matrix, 
* This works by finding the location of various uniform variables in the
* vertex shader, and transforming each vertex.
*
* @return void
*/
void SetupMVPMatrix(){
...
* SetupObjectTexture() 
* 
* Bind the texture slots to the samplers.
* Observe that we first select the texture we want (e.g. slot 0 shown below),
* then we find the location by name in the shader, and set the texture
* slot that we have binded to that location through the glUniform1i command.
*
* @return void
*/
void SetupObjectTexture(){
		...
		std::cout << "u_textureLocation:" << u_textureLocation << std::endl;
...
void SetupLights(){
    ...
    glm::vec3 lightPosition(0.0f,0.5f, 1.0f); // Pushed slightly behind the camera 


    GLint u_ambientIntensityLocation= glGetUniformLocation(gGraphicsPipelineShaderProgram,"u_ambientIntensity");
    glUniform1f(u_ambientIntensityLocation,ambientIntensity);
    

    // Setup our uniform for our light 
    GLint u_lightLocation= glGetUniformLocation(gGraphicsPipelineShaderProgram,"u_lightColor");
    glUniform3f(u_lightLocation,lightColor[0],lightColor[1],lightColor[2]);
    ...
    if(u_lightLocation>=0){
        // Setup the slot for the texture
    }else{
        std::cout << "Could not find u_lightColor, maybe a misspelling?" << std::endl;
        exit(EXIT_FAILURE);
    }
}
